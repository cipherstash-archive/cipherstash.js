...
class StashOauth {
  public async authenticateViaClientCredentials(
    ...
    const promise = makeOauthClient(idpHost).post(
      "/oauth/token",
  ...
  public async performTokenRefresh(
    ...
    if (response.ok) {
      if (response.value.status >= 200 && response.value.status < 400) {
        try {
          ...
          return Err(
            AuthenticationFailure(
              OAuthFailure(PlainError(`Failed to parse response from Auth0 (response body: ${response.value.data})`))
            )
          )
        }
      } else {
        return Err(
          AuthenticationFailure(
            OAuthFailure(PlainError(`Authentication failed - returned status ${response.value.status}`))
          )
        )
      }
    } else {
...
function camelcaseKeys(json: object): object {
  return Object.fromEntries(
    Object.entries(json).map(([key, val]) => [key.replace(/(\_\w)/g, k => k[1]!.toUpperCase()), val])
  )
}
