...
export class ConsoleAccessKeyStrategy
  implements AuthStrategy<OauthAuthenticationInfo>
{
  ...
  public async getAuthenticationDetails(): AsyncResult<
  ...
  private async acquireAccessToken(): AsyncResult<void, AuthenticationFailure> {
    const oauthInfo = await stashOauth.authenticateViaConsoleAccessKey(
      this.profile.config.identityProvider.accessKey,
    );
