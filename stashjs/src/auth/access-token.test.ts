...
describe("validateAccessToken", () => {
  it("should fail to decode an invalid token", () => {
    expect(validateAccessToken("not-a-real-token", "")).toEqual(errWithMessage("Failed to decode token"))
  })

  it("should fail if token contains no scope", () => {
    expect(validateAccessToken(createToken({}), "")).toEqual(errWithMessage("Token scope was not a string"))
  })
  ...
  it("should fail if token doesn't include principal tags when using workspace", () => {
    ...
    ).toEqual(errWithMessage('Token field "https://aws.amazon.com/tags" was not an object'))
  })
  ...
  it("should fail if principal tags don't include workspace", () => {
    ...
    ).toEqual(errWithMessage("Principal tags did not include workspace tag: ws:my-workspace"))
  })
  ...
  it("should fail if token scope does node include workspace", () => {
    ...
    ).toEqual(errWithMessage("Token scope did not include workspace tag: ws:my-workspace"))
  })
