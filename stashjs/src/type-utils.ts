type SupportedType = number | bigint | string | boolean | Date | { [key: string]: SupportedType }
 ...
 */
export type FieldType<R, S extends Field<R>> = S extends `${infer A}.${infer B}`
  ? A extends keyof R
    ? B extends Field<R[A]>
      ? FieldType<R[A], B>
      : never
 ...
 */
type FindFieldsOfType<R, S extends Field<R>, T> = { [F in S]: T extends FieldType<R, F> ? F : never }[S]
