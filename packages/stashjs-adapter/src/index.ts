...
export type QueryCallback<T> = (
  where: QueryBuilder<CSType<T>, MappingsWrapper<T>>
) => Query<CSType<T>, MappingsWrapper<T>>
...
export class CollectionAPI<T extends { id: number; stashId?: string | null }> {
  ...
  constructor(name: string, plaintextStore: RecordMapper) {
    ...
    this.#collection = Stash.connect().then(stash => stash.loadCollection<CSType<T>>(name))
  }
  ...
  async query(
    ...
    return result.documents.map(record => ({ ...record, id: record.originalId } as unknown as T))
  }
