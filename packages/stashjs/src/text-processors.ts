...
export const standardTokenizer: TextProcessor = input => input.flatMap(t => t.split(/[ ,;:!]/))
...
export const ngramsTokenizer =
  ...
  input =>
    input
      .flatMap(t =>
        range(0, Math.max(t.length - config.tokenLength, 0))
          .map((_, index) => t.slice(index, index + config.tokenLength))
          .flat()
      )
      .filter(token => token.length == config.tokenLength)

export const downcaseFilter: TextProcessor = input => input.flatMap(t => t.toLowerCase())

export const upcaseFilter: TextProcessor = input => input.flatMap(t => t.toUpperCase())

export const textPipeline = (tokenizers: Array<TextProcessor>): TextProcessor =>
  tokenizers.reduce(
    (acc, tokenizer) => compose(tokenizer, acc),
    input => input
  )
