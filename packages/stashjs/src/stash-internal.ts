...
export class StashInternal {
  ...
  private constructor(
    ...
    this.sourceDataCipherSuiteMemo = profile.withFreshKMSCredentials<CipherSuite>(async awsConfig => {
      return Ok.Async(
        makeCipherSuite(makeNodeCachingMaterialsManager(this.profile.config.keyManagement.key.arn, awsConfig))
      )
    })
  }
  ...
  public static async connect(maybeProfile?: StashProfile): AsyncResult<StashInternal, ConnectionFailure> {
    ...
    const refGenerator = await profile.value
      .withFreshKMSCredentials<MakeRefFn>(async awsConfig => {
        const result = await makeRefGenerator(new KMS(awsConfig), profile.value.config.keyManagement.key.namingKey)
        if (result.ok) {
    ...
    if (refGenerator.ok) {
      ...
      return Ok(
        new StashInternal(stub, makeAsyncResultApiWrapper(stub, profile.value), profile.value, refGenerator.value)
      )
    } else {
  ...
  private async unpackCollection<R extends StashRecord, M extends Mappings<R>, MM extends MappingsMeta<M>>(
    ...
    return await sequence(
      ...
      ([collectionMeta, mappings, mappingsMeta]) =>
        Ok.Async(
  ...
  private async decryptMappings(
    encryptedMappings: V1.Index.IndexOutput[]
  ): AsyncResult<Array<StoredMapping>, DecryptionFailure> {
    return await sequence(
      _ => convertAsyncErrorsTo(DecryptionFailure, this.sourceDataCipherSuiteMemo.freshValue()),
      async cipher =>
        gather(
          await Promise.all(
            encryptedMappings.map(async em =>
              gatherTuple2([await cipher.decrypt<StoredMapping>(em.settings!), Ok(em.id!)])
            )
          )
        ),
      decrypted =>
        Ok.Async(
          decrypted.map(([{ mapping, meta }, indexId]) => ({
            mapping,
            meta: {
              ...meta,
  ...
  private async encryptMappings<R extends StashRecord, M extends Mappings<R>, MM extends MappingsMeta<M>>(
    ...
    if (memo.ok) {
      const encryptedIndexes = await Promise.all(
        Object.entries(definition.mappings).map(async ([indexName, mapping]) => {
  ...
  private async decryptResponses(
    res: V1.Collection.ListReply
  ): AsyncResult<Array<CollectionMetadata>, DecryptionFailure> {
    return await sequence(
      _ => convertAsyncErrorsTo(DecryptionFailure, this.sourceDataCipherSuiteMemo.freshValue()),
      async cipher =>
        Ok(
