 ...
 */
export async function retryPromise<T>(callback: () => Promise<T>, options: RetryOptions): Promise<T> {
  ...
  while (true) {
    try {
      ...
      if (options.retryOn(e)) {
        ...
        const waitTime = Math.min(Math.pow(2, count) * 100 + jitter, options.maxBackoffMs ?? DEFAULT_MAXIMUM_BACKOFF_MS)
