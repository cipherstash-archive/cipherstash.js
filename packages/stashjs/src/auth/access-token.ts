...
export function validateAccessToken(
  ...
  for (const expected of expectedScope.split(/\s+/).filter(x => !!x.trim())) {
    if (!scopes.has(expected)) {
      return Err(TokenValidationFailure(`Token did not have expected scope: ${expected}`))
    }
  }
  ...
  if (workspace) {
    ...
    if (!isObject(amazonTags)) {
      return Err(TokenValidationFailure(`Token field "https://aws.amazon.com/tags" was not an object`))
    }

    const principalTags = amazonTags["principal_tags"]

    if (!isObject(principalTags)) {
      return Err(TokenValidationFailure(`Token field "principal_tags" was not an object`))
    }

    const tags = principalTags["workspace"]

    if (!Array.isArray(tags)) {
      return Err(TokenValidationFailure(`Token principal_tags did not exist or was not an array`))
    }

    const workspaceTag = `ws:${workspace}`

    if (!tags.includes(workspaceTag)) {
      return Err(TokenValidationFailure(`Principal tags did not include workspace tag: ${workspaceTag}`))
    }

    if (!scopes.has(workspaceTag)) {
      return Err(TokenValidationFailure(`Token scope did not include workspace tag: ${workspaceTag}`))
    }
  }
