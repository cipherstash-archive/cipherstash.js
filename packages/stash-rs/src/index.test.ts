...
describe("Compare (big-int)", () => {
  test("compare greater than", () => {
    ...
    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(456)), ore.encrypt(ORE.encodeNumber(100)))).toEqual(1)
  })

  test("compare less than", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(100)), ore.encrypt(ORE.encodeNumber(788881001)))).toEqual(-1)
  })

  test("compare equal", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(100888)), ore.encrypt(ORE.encodeNumber(100888)))).toEqual(0)
  })

  test("compare equal 0", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(0)), ore.encrypt(ORE.encodeNumber(0)))).toEqual(0)
  })
})
...
describe("Compare (number)", () => {
  ...
  test("compare less than", () => {
    ...
    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(100)), ore.encrypt(ORE.encodeNumber(788881001.75)))).toEqual(-1)
  })

  test("compare equal", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(10088)), ore.encrypt(ORE.encodeNumber(10088)))).toEqual(0)
  })
  ...
  test("compare different fractional component", () => {
    ...
    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(800.3)), ore.encrypt(ORE.encodeNumber(800.7)))).toEqual(-1)
  })

  test("compare shifted decimal point", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(80000.75)), ore.encrypt(ORE.encodeNumber(800.0075)))).toEqual(1)
  })

  test("compare negative to positive", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(-800)), ore.encrypt(ORE.encodeNumber(700)))).toEqual(-1)
  })

  test("compare negative to negative", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(-800)), ore.encrypt(ORE.encodeNumber(-900)))).toEqual(1)
  })

  test("compare negative to negative with different fractional components", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(-800.3)), ore.encrypt(ORE.encodeNumber(-800.766)))).toEqual(1)
  })

  test("compare negative to negative with shifted decimal point", () => {
    let ore = ORE.init(k1, k2)

    expect(ORE.compare(ore.encrypt(ORE.encodeNumber(-80076.6)), ore.encrypt(ORE.encodeNumber(-800.766)))).toEqual(-1)
  })
})
